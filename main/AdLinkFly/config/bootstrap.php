...
\Cake\Event\EventManager::instance()->on('Auth.afterIdentify', function ($event, $result, $auth) {
    $users = \Cake\ORM\TableRegistry::getTableLocator()->get('Users');
    /**
     * @var \App\Model\Entity\User $user
     */
    $user = $users->get($result['id']);
    $user->setDirty('modified', true);
    $user->login_ip = get_ip();
    $users->save($user);
});

\Cake\Event\EventManager::instance()->on('SocialAuth.afterIdentify', function ($event, $result) {
    $users = \Cake\ORM\TableRegistry::getTableLocator()->get('Users');
    /**
     * @var \App\Model\Entity\User $user
     */
